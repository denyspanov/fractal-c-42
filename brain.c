...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/05 16:23:03 by dpanov            #+#    #+#             */
/*   Updated: 2017/04/05 16:23:13 by dpanov           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

void	norm_helper_brain(t_fractal *fract, t_data *data, int y_start)
{
...
void	brain(t_data *data, int x_start, int x_end, int y_start)
{
	...
	while (++y_start - 1 < data->win_height)
	{
		...
		while (++fract->s < x_end)
		{
			norm_helper_brain(fract, data, y_start);
			while (fract->n_c_re * fract->n_c_re + fract->n_c_im *
			fract->n_c_im < 4 && ++fract->iteration < data->max)
			{
				fract->o_c_re = fabs(fract->n_c_re);
				fract->o_c_im = fabs(fract->n_c_im);
