...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/05 17:44:28 by dpanov            #+#    #+#             */
/*   Updated: 2017/04/05 17:45:50 by dpanov           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

void	norm_helper_brainv(t_fractal *fract, t_data *data, int y_start)
{
	fract->c_re = -0.7 * data->mouse_x;
	fract->c_im = 0.27015 * data->mouse_y;
	fract->n_c_re = 1.5 * (fract->s - data->win_width / 2) /
		(0.5 * data->win_width);
	fract->n_c_im = (y_start - data->win_height / 2) /
(0.5 * data->win_height);
...
void	brain_v(t_data *data, int x_start, int x_end, int y_start)
{
	...
	while (++y_start - 1 < data->win_height)
	{
		...
		while (++fract->s < x_end)
		{
			norm_helper_brainv(fract, data, y_start);
						...
						fract->n_c_im < 4 && ++fract->iteration < data->max)
			{
				...
				fract->n_c_re = fabs(fract->o_c_re * fract->o_c_re -
